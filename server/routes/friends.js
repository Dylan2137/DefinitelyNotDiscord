...
router.post('/get-friends', async(req, res) => {
    ...
    try{
        ...
        const [friends] = await db.execute("SELECT room_id, login, user_id, profile_picture FROM rooms JOIN users ON (users.user_id = rooms.user1_id) OR (users.user_id = rooms.user2_id) WHERE (rooms.user1_id = ? OR rooms.user2_id = ?) AND login != ?", [userId, userId, login]);
